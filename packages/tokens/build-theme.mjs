 ...
 * actually differs*, scoped to [data-ds-theme="<name>"].
 *
...
try {
  ...
  console.error(`@ds/tokens: theme "${theme.name}" does not resolve\n`);
...
if (errors.length) {
  console.error(`@ds/tokens: ${theme.name} breaks ${errors.length} contrast promise(s)\n`);
...
if (CHECK_ONLY) {
  console.log(
    `@ds/tokens: ${theme.name} holds all ${usage.pairings.length} contrast promises ` +
  ...
  console.log(
    `@ds/tokens: theme "${theme.name}" → ${path.relative(process.cwd(), target)} ` +
      `(${changedLight.length} light / ${changedDark.length} dark overrides, ` +
